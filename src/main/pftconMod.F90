module pftconMod
  ...
  type, public :: pftcon_type
     ...
     real(r8), allocatable :: arootf        (:)   ! parameter used in CNAllocation of fruit trees (added by O.Dombrowski)
     ...
     real(r8), allocatable :: aleafstor     (:)   ! parameter used in CNAllocation of fruit trees (added by O.Dombrowski)
     ...
     real(r8), allocatable :: transplant    (:)   ! parameter used in CNPhenology of fruit trees (added by O.Dombrowski)
     real(r8), allocatable :: lfmat         (:)   ! parameter used in CNPhenology of fruit trees (added by O.Dombrowski)
     real(r8), allocatable :: grnrp         (:)   ! parameter used in CNPhenology of fruit trees (added by O.Dombrowski)
...
contains
  ...
  subroutine InitRead(this)
    ...
    call ncd_io('crequ', this%crequ, 'read', ncid, readvar=readv) ! O.Dombrowski
    if ( .not. readv ) call endrun(msg=' ERROR: error in reading in pft data'//errMsg(sourcefile, __LINE__))

    call ncd_io('crit_temp', this%crit_temp, 'read', ncid, readvar=readv) ! O.Dombrowski
    if ( .not. readv ) call endrun(msg=' ERROR: error in reading in pft data'//errMsg(sourcefile, __LINE__))

    call ncd_io('ndays_stor', this%ndays_stor, 'read', ncid, readvar=readv) ! O.Dombrowski
    if ( .not. readv ) call endrun(msg=' ERROR: error in reading in pft data'//errMsg(sourcefile, __LINE__))

    call ncd_io('prune_fr', this%prune_fr, 'read', ncid, readvar=readv) ! O.Dombrowski
    if ( .not. readv ) call endrun(msg=' ERROR: error in reading in pft data'//errMsg(sourcefile, __LINE__))
